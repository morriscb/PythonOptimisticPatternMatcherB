...
class OptimisticPatternMatcherB(object):
    ...
    def _construct_and_match_pattern(self, source_candidates, n_match):
        ...
        for dist_idx in xrange(start_idx, end_idx):
            ...
            # the user specified max rotation. This is the major rejection
            ...
            if dot > 0:
                ...
                matched_references.append(self._id_array[dist_idx, 1])
                matched_references.append(self._id_array[dist_idx, 0])
            # One quick test we can do now that we have a source and a
            # reference candidate for that source is check if the absolute
            # distances in x and y between the two are within our max allowed
            # shift. If they are not we can discard the this reference pair.
            ...
            if (np.fabs(source_candidates[0, 0] - x) >= self._max_shift or
                ...
                continue
            # Now that we have a firm reference candidate for the first spoke
            # of our  pinwheel we can start computing quantities we will need
            # for finding the remaining spokes. Our convention is that we
            # rotate the source vector into the reference vector.
            ...
            # We want to check if the other spokes of our pinwheel rotate in
            # the correct direction, however if our candidate rotation is
            # already close to zero within tolerance we can avoid checking for
            # this.
    ...
    def _pattern_spoke_test(self, cand_dist, cand_dx, cand_dy, ref_center_id,
        ...
        hold_id = -99
        for dist_idx in xrange(start_idx, end_idx):
            # Again the dot produt is a great way to test that our vector
            # points in the correct diretcion.
            ...
            if (np.sign(dot) < 0 and
                not ref_id_array[dist_idx, 1] == ref_center_id):
                continue
            ...
            if ((not cross_approx_zero and
                ...
                # We found a match, store the correct id and then exit the
                # loop.
                if np.sign(dot) < 0:
        ...
        for pattern_idx in xrange(np.min((self._max_n_patterns,
            ...
            if len(ref_candidates) >= n_match:
                ...
                if len(matches) > self._min_matches:
                    ...
                    print("\tShift: %.2f %.2f pix" %
                          (pattern[0][0] -
                           self._reference_catalog[ref_candidates[0]][0],
                           pattern[0][1] -
                           self._reference_catalog[ref_candidates[0]][1]))
                    print("\tRotation: %.4f deg" %
                          (np.arcsin(sin_theta)/__deg_to_rad__))
                    break
        if matches is None:
